...
class MakeTenantModuleCommand extends Command
{
    protected $signature = 'make:tenant:module {name}';
    protected $description = 'Create a new tenant module';

    public function handle()
    {
        $name = $this->argument('name');
        $moduleName = Str::studly($name);
        $modulePath = app_path("Modules/Tenant/{$moduleName}");
        ...
        $this->info("Tenant module {$moduleName} created successfully!");
    }
    ...
    protected function addPermissionsToSeeder($name)
    {
        ...
        $this->info("Permissions for {$name} module added to Tenant PermissionsSeeder.php");
    }
    ...
    protected function updatePostmanCollection($moduleName)
    {
            ...
            'item' => [
                [
                    ...
                    'request' => [
                        ...
                        'url' => [
                            'raw' => '{{base_url}}/api/tenant/' . $moduleLower,
                            'host' => ['{{base_url}}'],
                            'path' => ['api', 'tenant', $moduleLower]
                        ],
                ...
                [
                    ...
                    'request' => [
                        ...
                        'url' => [
                            'raw' => '{{base_url}}/api/tenant/' . $moduleLower . '/{{id}}',
                            'host' => ['{{base_url}}'],
                            'path' => ['api', 'tenant', $moduleLower, '{{id}}']
                        ],
        ...
        // Add the new folder to the Tenant folder
        $tenantFolder['item'][] = $newFolder;

        // Save the updated collection
        file_put_contents($postmanPath, json_encode($collection, JSON_PRETTY_PRINT));

        $this->info("Postman collection updated with Tenant {$moduleName} module routes");
    }
} 